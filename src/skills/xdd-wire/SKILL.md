...
本文件（xdd-wire）只补充 xdd 特有：上下游绑定 + 路径约定 + RXX 溯源 + xdd 自检清单。

## xdd 上下游绑定

| | |
|---|---|
| **上游** | `xdd-brainstorm`(intent.md) + `xdd-spec`(RXX + Feature 页面名/交互/角色) |
| **我产出** | `.xdd/design/wire/{page}.md`（含 SVG 布局 + 元素清单 + 9 操作态 + Review） |
| **下游** | `xdd-plan`（前端 task）、`xdd-verify`（页面渲染验收） |
...
通用 wire-design 的 wire 文档 vs xdd-wire 的 wire 文档，关键差异：

| 维度 | 通用 wire-design | xdd-wire（叠加） |
|------|------------------|------------------|
| 路径 | 任何 `{project}/docs/wire/` 或类似 | `.xdd/design/wire/`（业务规范） |
| 元素追溯 | 可选 `@covers-{bxx}-RXX` | 必须 `@covers-RXX`（按 Bxx-slug 隔离） |
| 自检 | 8 条关注点层 | 8 + 6 = 14 条（叠加） |
| 上下游 | 无绑定 | 显式绑定 xdd-brainstorm/spec/plan/verify |
...
| 阶段 | xdd skill | 产出 |
|------|-----------|------|
| 设计·理解 | xdd-brainstorm | intent.md |
| 设计·规格 | xdd-spec | spec/{Bxx-slug}/rules.md + scenarios.feature |
| 设计·架构 | xdd-architecture | architecture/{Bxx-slug}/ + flow.mermaid |
| **设计·前端** | **xdd-wire** | **wire/{page}.md** ← 你在这 |
| 设计·韧性 | xdd-resilience | resilience/{Bxx-slug}/ |
| 桥接·计划 | xdd-plan | plan/{Bxx-slug}/plan.md |
| 代码·实现 | xdd-execute | 代码 @implements RXX |
| 代码·验证 | xdd-verify | verify-report.md |
