...
description: |
  ...
  architecture 的结构锚配套验证工具（流程图画完必跑）。适用任何 Mermaid 语法验证场景。
...
```bash
bash skills/mermaid-check/scripts/mmdc_check.sh [.xdd 路径]
```
...
# 检查指定项目
bash skills/mermaid-check/scripts/mmdc_check.sh /path/to/project/.xdd
...
- 纯工具 skill，不做业务设计
- 不修改任何文件
- architecture 画完 flow.mermaid 后建议跑此验证
- 可独立使用
...
| 技能 | 边界说明 |
|------|----------|
| architecture | 产出 `flow.mermaid`；mermaid-check 是它的渲染验证工具，不做流程设计 |
| verify | 全链路一致性审计可调用本工具确认图表可渲染 |
