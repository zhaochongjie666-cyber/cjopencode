...
| | |
|---|---|
| **上游** | `plan`（`.xdd/runs/xdd_run/plan.md` task DAG + 冻结的 `qa-plan.md` 公开入口测试契约） |
| **我产出** | 代码（每处 `@implements RXX`）+ 测试（每个 RXX 至少 1 个）+ 执行报告 |
| **下游消费者** | `verify`（按 Feature 验收 + 真实可用契约） |
| **回溯锚** | 代码注释 `@implements RXX` ← plan task ← spec 规则 ← design 意图 |

## 怎么做

读 task 的 `**Stack:**` 字段，按栈装专项 skill：
- `backend`  → 装 `backend`（加载 `.xdd/rules/backend.rules` + 后端检查：DB 迁移/端点/事件/事务）
- `frontend` → 装 `frontend`（加载 `frontend.rules`+`ui-ux.rules` + 前端检查：对照 wire 6 态/600行）
...
- [ ] Docker 服务起来（`docker compose up -d --wait`，每个 healthcheck 过）—— 用 `architecture` 的 docker-compose.yml；缺 DB/队列/缓存就拉起或配置 test 替代
...
**只读 Code Review 自审**：提交 execute 产物时，`artifacts` 必须包含生产源码路径，不能只交 plan/docs/tests。用 `read` 读取全部生产源码，从 6 个角度逐文件自审（空值安全 / 并发安全 / 资源生命周期 / 授权与注入 / 错误处理 / 架构漂移），把审查结果 `write` 到 `.xdd/runs/xdd_run/code-review.json`（含 artifactPaths + artifactDigest + checks 六维度 + verdict）。自审不是独立审查 -- xdd-flow 验收时可用 `task` 派 general subagent 复审。
...
```bash
bash skills/execute/scripts/no-stub-check.sh <你刚改的文件或目录>
...
on_failure(n):                          # n = 同一 task 连续失败次数
  ...
  elif n == 3: 退一步回 plan 重规划（RXX 映射错 / 端点漏 / 依赖冲突；或更上 design/spec 层找根因）
...
凡 `.xdd/runs/` 内产物（执行报告 / failure-log / Plan update）：叙述用**文言短语 ＋ 符号**，弃白话长句。义不损，字求简（见 SYSTEM_AGENTS.md §7）。

- 符号：流转 `→`；推导 `⇒`（如 `task 三败 ⇒ HALT 上报`）；并列 `＋`；定义 `＝`；章节引 `§`；比较 `≤ ≥ ≠`
- **例外（机器格式勿文言）**：命令、代码、Evidence 原始输出、路径、表格列名
