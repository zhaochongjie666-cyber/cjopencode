...
| | |
|---|---|
| **上游** | `execute`（代码 + 测试）+ 冻结 `qa-plan.md` + 全部设计层锚：spec/architecture/wire/resilience |
...
8. **运行时观测（适用时）** —— 有可部署 runtime 时，用 `bash` 跑 healthcheck/curl 收集指标，`read` 对比 `.xdd/runs/xdd_run/runtime-observability/baseline.json` 基线，`write` 保存观测到 latest.json + incident.json（脱敏后落盘）；库/CLI 等没有可观测 runtime 时软跳过，不能伪造指标。P1 回归必须回炉，P2 保留软告警。
9. **质量评分** —— 用 `read`/`glob` 检查证据文件存在性（qa-plan.md / verify-report.md / code-review.json / commit-review.json / incident.json），用 `bash` 跑 `git status --porcelain` 检查工作区干净度。基于证据覆盖率、缺陷逃逸数、重复缺陷率做定性评分，`write` 到 `.xdd/runs/xdd_run/quality-score.json`（含 score 0-100 + status + metrics + recommendations）。评分只提供改进优先级，不形成第二个无限硬 Gate。
10. **最终聚合裁决** —— 用 `read`/`grep` 逐项检查 gate 条件：QA 证据齐？code-review.json 存在且 checks 全 pass？git worktree 干净？runtime 观测无 P1（或按不适用软跳过）？verify 证据齐？全过 = release，任一不过 = BLOCK 并按失败项回炉。`write` 裁决到 `.xdd/runs/xdd_run/release-decision.json`（含 verdict release/block + checks 逐项 ok/reason + inputDigest）。只有 verdict=release 才能最终推进。
...
跑 `resilience/chaos-scenarios.md` 的 P0 子集，验兜底真生效（用 `resilience/scripts/chaos-runner.sh`）：
...
```bash
bash skills/execute/scripts/no-stub-check.sh .
# 全项目零存根/零假实现
```
...
for round in 1..3:
  if round == 1: 修报告里的 P0 + P1（改代码）→ 重跑健康检查+漫游
    ...
    if 仍有 P1:                       # 修不动 = 找根因
      rollback(根因):                 # 判定见括号
        实现缺陷（代码 bug / 端点缺失 / 测试失败）-> execute（主动返回修复后重跑 verify）
        规则没写清（spec RXX 含糊/冲突）  → spec
        空状态/页面缺（wire/{page}.md 缺该状态）→ wire
        工作流卡点（design.md 该决策缺失）  → brainstorm
        API/事件错（architecture.md 没覆盖）→ architecture
        兜底不够/错（resilience/ 没覆盖该失败模式）→ resilience
...
凡 `.xdd/runs/` 内产物（验证报告 / 结论 / failure 处理记录）：叙述用**文言短语 ＋ 符号**，弃白话长句。义不损，字求简（见 SYSTEM_AGENTS.md §7）。

- 符号：流转 `→`；推导 `⇒`（如 `证据缺 ⇒ 判 ❌`）；并列 `＋`；定义 `＝`；章节引 `§`；比较 `≤ ≥ ≠`
- **例外（机器格式勿文言）**：命令、代码、证据原始输出（截图路径/响应）、表格列名
