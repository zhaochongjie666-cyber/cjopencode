...
| | |
|---|---|
| **上游** | `brainstorm`(design.md) + `spec`(RXX 规则) |
| **我产出** | `{bxx-slug}/architecture.md` + `flow.mermaid` + `docker-compose*.yml`；全局 `aggregate-landscape.md` + `event-contract.md` |
| **下游消费者** | `plan`（端点/聚合/事件 → task）、`resilience`（ODD 失败模型是韧性种子）、`verify`（端点清单做 architecture↔code 审计） |
...
- 运行时状态，不参与设计内容评审冻结；可由 `verify/scripts/sync-contract-checkboxes` 半自动翻转
...
└── {bxx-slug}/
    ...
    └── resilience/              # resilience 产出（见该 skill）
```
...
□ flow.mermaid 能渲染（用 mermaid-check 验）？
