...
| **下游消费者** | 之后可正常走 `plan` / `execute` / `verify` 改这个项目 |
...
- 产出 `.xdd/design/spec/{bxx-slug}/rules.md` + `*.feature`（反推版）。Gherkin 语法/具体值写法 → 详见 `gherkin-plus` skill。
...
if 要改功能:
  brainstorm(基于反推的 design.md 继续) -> spec -> ... -> execute
elif 要补追溯:
