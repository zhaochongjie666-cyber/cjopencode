...
| | |
|---|---|
| **上游** | `architecture`（架构 + 运维视图 §失败模型 = 韧性种子）+ `spec`（行为基线，找它的反面） |
| **我产出** | `architecture/{bxx-slug}/resilience/` 5 文档 |
| **下游消费者** | `plan`（兜底约束 + 失败注入点写进 task）、`verify`（混沌演练验兜底） |
...
- 运行时状态，不参与韧性设计内容评审冻结；可由 `verify/scripts/sync-contract-checkboxes` 半自动翻转
...
`chaos-scenarios.md` 把高优先级失败模式翻译成可执行 Gherkin。强制标签：`@chaos` + `@failure-mode-FXX` + `@P0/@P1` + `@covers-RXX`。Gherkin 语法/具体值写法 → 详见 `gherkin-plus` skill。
