...
prompt ->
  design_layer():                    # 设计层（锚）—— 每个产物带「上游指针 + 下游消费者」
    understand(use skill: brainstorm)
    -> spec(use skill: spec, RXX)
    -> architecture(use skill: architecture)
    -> wire(use skill: wire)
    -> resilience(use skill: resilience)
  -> plan(use skill: plan, 每个 task 回指 RXX)        # 桥接
  -> code_layer():
       execute(use skill: execute)
       -> verify(use skill: verify)
...
```
design_layer():
  understand(use skill: brainstorm) -> intent.md + design.md(5段决策)   # 锚定：意图
                   >> 用户审查闸：design.md 写完停下，用户确认意图对齐才进 spec（防偏第一道闸）
  spec(use skill: spec)             -> spec/{bxx-slug}/ rules.md + *.feature  # 锚定：规则 RXX
  architecture(use skill: architecture) -> architecture/{bxx-slug}/ architecture.md + flow.mermaid + 端点/事件契约 + 运维视图   # 锚定：结构
  wire(use skill: wire)             -> wire/{page}/ 6 操作态 + review.md  # 锚定：前端（纯后端跳过）
  resilience(use skill: resilience) -> architecture/{bxx-slug}/resilience/ 5 文档  # 锚定：韧性
```
...
| skill | 做什么 | 核心纪律 |
|-------|--------|---------|
| `execute` | 按 plan task 写代码（TDD），代码 `@implements RXX` 回指 | 无存根/无假实现/跑通有证据 |
| `verify` | 真实验证：能跑/数据落地/页面开/无存根/双契约 | 禁偷懒归因 + 失败穷举 ≥3 假设 + 4 维一致性审计 |
...
```
trace_chain():
  intent.md(why)                 <- understand(use skill: brainstorm)
     |
  design.md(决策 5 段)            <- understand(use skill: brainstorm)
     |
  spec/ RXX 规则(做什么)          <- spec(use skill: spec)
     |
  architecture.md(结构+API+事件)  <- architecture(use skill: architecture)
     |
  plan.md task(回指 RXX)          <- plan(use skill: plan)
     |
  代码 @implements RXX            <- execute(use skill: execute)
     |
  verify 运行证据                 <- verify(use skill: verify)
...
## 10. 行文之法（.xdd 一律文言式 ＋ 符号）

凡 `.xdd/` 内文档：叙述用**文言短语**，逻辑用**符号**，弃白话长句。义不损，字求简。
