...
description: |
  ...
  触发：初始化、init、新项目、init、起项目、开始、脚手架骨架。
---
...
work():
  ...
  5. 下一步                -> 启动 xdd-flow，进 brainstorm
```
...
```bash
# 任何项目目录下：
bash skills/init/scripts/init.sh
# 或装好后：bash ~/.config/opencode/skills/init/scripts/init.sh
```
...
├── design/                    ← 设计层（持久锚，长期保留）
│   ├── intent.md              ← 【项目层】意图锚：项目总意图（跨业务线共享），brainstorm 填
│   ├── design.md              ← 【项目层】收敛决策：项目级总决策（跨业务线的全局决策），brainstorm 填
│   ├── notes/                 ← 发散笔记（glossary 持久；其余设计期）
│   ├── spec/                  ← 【业务线层】规则锚 RXX + Gherkin（spec 填；语法详见 gherkin-plus）
│   │   ├── _landscape.md      ← 业务线全景（--bizlines 时生成）
│   │   └── BXX/business.md    ← 业务线占位（始终用 BXX；--bizlines 时按参数生成，否则 B01）
│   ├── architecture/          ← 【业务线层】结构锚 colocation（architecture 填）
│   └── wire/                  ← 【业务线层】前端锚（wire 填，纯后端跳过）
├── runs/                      ← 【运行层】单轮工作记录（单 run 一份）
│   └── xdd_run/
│       ├── status.md          ← 本 run 进度（3 层 × 业务线，✅/⏳）
│       ├── goals.md           ← 本 run 高层目标清单（ACK G 区索引源，动态追加）
│       ├── plan/{bxx-slug}/          ← 本 run 的 TDD task DAG（plan 填）
...
| 设计·前端 | ⏳ | wire | design/wire/{page}.md |
...
1. **只生骨架** — init 不写 design.md 内容（那是 brainstorm 的活），只生占位 + 目录结构。
2. **入口路由判定** — 检测存量代码（源码/项目配置/git 跟踪文件）→ 警告指向 `reverse`，`--force` 才继续。不让 init 盲目 scaffold 进遗留项目。
...
| 有存量代码（无 `.xdd/`）| **警告 + exit**（指向 reverse）| `reverse`（反推设计 + 追溯）|
...
# xdd-flow 第一步装 brainstorm 写 design/intent.md + design.md
```
...
| "检测到存量代码" | 仓库已有源码 | 改用 `reverse`，或 `--force` 确认新项目 |
