...
# 平台中立，无 hook 依赖。详见 skills/init/SKILL.md
...
if [ -d ".xdd" ]; then
  if [ "$FORCE" = "true" ]; then
    ...
    echo "❌ .xdd/ 已存在。xdd 只使用固定 run 目录 .xdd/runs/xdd_run；继续已有项目请直接用 下一个 skill，或 --force 重建运行骨架。"
    exit 1
  fi
else
  # 全新项目：存量代码检测
  if detect_existing_codebase && [ "$FORCE" != "true" ]; then
    ...
    echo "   这个仓库可能已有代码。如果是遗留项目要补设计文档，应该用 reverse（反推设计 + 追溯）。"
    echo "   确认是从零 scaffold 新项目？加 --force 继续，或换 reverse。"
    exit 1
  fi
fi
...
# intent.md（意图锚占位，brainstorm 填）
cat > .xdd/design/intent.md <<'EOF'
# 意图锚 — {项目名}

> 用户为什么做这个？要解决谁的什么问题？不行的话现状痛在哪？
> 这一层是**用户审的契约**——确认对齐才往下。brainstorm 填。
...
> 业务线占位。spec 填：目标 / 关键问题 / 范围 / 通用语言引用 / 关联。
EOF2
  done
...
> 默认业务线占位（始终用 BXX-slug 结构）。spec 填：目标 / 关键问题 / 范围 / 通用语言引用 / 关联。
...
| 设计·规则 | ⏳ | spec | design/spec/{bxx-slug}/ rules.md + *.feature |
| 设计·架构 | ⏳ | architecture | design/architecture/{bxx-slug}/ |
| 设计·前端 | ⏳ | wire | design/wire/{page}/（纯后端跳过）|
| 设计·韧性 | ⏳ | resilience | design/architecture/{bxx-slug}/resilience/ |
| 桥接·计划 | ⏳ | plan | runs/xdd_run/plan/{bxx-slug}/plan.md |
...
# 从 template 复制（G 编号由 brainstorm 生成分配）
...
echo "xdd-flow 第一步会装 brainstorm 写 design/intent.md + design.md"
