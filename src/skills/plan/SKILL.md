...
| | |
|---|---|
| **上游** | `brainstorm`(design.md 意图) + `spec`(spec/{bxx-slug}/ RXX 规则 + Feature) + `architecture`(architecture/{bxx-slug}/ 端点/事件/状态机/文件清单) + `wire`(wire/{page}.md 前端线框) + `resilience`(architecture/{bxx-slug}/resilience/ 兜底约束) |
| **我产出** | `.xdd/runs/xdd_run/plan.md`（任务 DAG）+ `.xdd/runs/xdd_run/qa-plan.md`（实现前冻结的公开入口 QA 契约） |
| **下游消费者** | `execute`（按 task 写代码，每个 commit 回指 RXX） |
...
1. `.xdd/design/spec/{bxx-slug}/*.feature` —— Feature/Scenario、Then/And 断言、异常路径、Scenario Outline + Examples（语法/具体值写法 → 详见 `gherkin-plus` skill）
...
`**Stack:**` 取 `backend` 或 `frontend`（来自 architecture 规则传导矩阵的列：后端文件→backend / 前端组件→frontend；纯后端项目全 backend）。task 的 Stack 字段决定 execute 装哪个专项 skill（`backend` / `frontend`）。
...
凡 `plan.md` 叙述（目标/约束/依赖说明/task 描述）：用**文言短语 ＋ 符号**，弃白话长句。义不损，字求简（见 SYSTEM_AGENTS.md §7）。

- 符号：流转 `→`（如 `红 → 绿 → commit`）；推导 `⇒`；并列 `＋`；定义 `＝`；章节引 `§`；比较 `≤ ≥ ≠`
- 例：白话「这个任务依赖任务二完成之后才能开始」⇒ 文言「本 task 依赖 T2 ⇒ T2 ✅ 后始」
- **例外（机器格式勿文言）**：TDD Steps 命令、代码/路径、表格列名、`**goal:** G1` / `@implements` 等标注
...
**动态计划约定**：执行者直接维护本文件，不另建一份 TODO/plan 造成双重事实源。task 开始即改 `[~]`；每个 Step 完成即改 `[x]` 并在该 Step 下追加 `Evidence: <命令 + 关键结果>`；假设、顺序或实现方式变化时追加带时间的 `Plan update`，说明“事实 / 拷问 / 决策 / 影响”。只允许原地更新进度、证据和不改变契约的微调；RXX、接口、依赖 DAG、文件范围或验收标准变化属于结构性变化，必须标 `[!]` 并回到 plan（必要时 design/spec/architecture）重规划后再执行。
