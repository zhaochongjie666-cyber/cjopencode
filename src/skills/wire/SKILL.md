...
凡 `.xdd/design/wire/{page}.md` 叙述：用**文言短语 ＋ 符号**，弃白话长句。义不损，字求简（见 SYSTEM_AGENTS.md §7）。

- 符号：流转 `→`（如 `提交 → 校验 → 成/败两态`）；推导 `⇒`；并列 `＋`；定义 `＝`；章节引 `§`；比较 `≤ ≥ ≠`
- **例外（机器格式勿文言）**：SVG/mermaid、页面可见文案（用户直接读到，仍白话）、表格列名、`@covers` 标注
