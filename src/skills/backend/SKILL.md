...
name: backend
description: |
  xdd 代码层专项 —— 后端实现锚。被 execute 按 task 的 Stack=backend 派发装入。
...
> 代码层专项：不取代 execute 的 TDD 主流程，只补后端栈特定的约定与检查。
...
| | |
|---|---|
| **上游** | `execute`（派发装入）+ `architecture.md §技术栈`（后端语言/框架/ORM 决策）+ `.xdd/rules/backend.rules`（项目级 WHAT 约定：分层/错误码/auth/命名） |
| **我产出** | 后端代码（`@implements RXX`）+ 栈特定检查通过 |
| **下游消费者** | `verify`（端点契约 + 真实持久化验收） |
...
本 skill 只管「后端怎么实现得扎实」；通用 TDD 流程（任务调度 / 阻塞处理 / 卡住升级）在 `execute`。
