...
10. 代码自审：用 `read` 读取刚写的全部生产源码，从 6 个角度逐文件审查（空值安全 / 并发安全 / 资源生命周期 / 授权与注入 / 错误处理 / 架构漂移）。把审查结果 `write` 到 `.xdd/runs/xdd_run/code-review.json`（含 artifactPaths + artifactDigest + checks 六维度 + verdict pass/fail/inconclusive）。自审不是独立审查 -- xdd-flow 验收时可用 `task` 派 general subagent 复审。
...
- [ ] 全量测试 PASS？
- [ ] code-review.json 含 6 维度 checks + artifactDigest 绑定当前源码？
- [ ] 清理完成：无调试代码 / 格式统一 / 无死代码 / README 反映最终接口？

## 完成后

回报 xdd-flow：执行报告（task 完成表 + RXX 覆盖 + 全量测试 + 提交历史 + 遗留事项）。
